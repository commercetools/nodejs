...
describe('createService', () => {
  ...
  describe('parse', () => {
    ...
    describe('features of parsed object', () => {
      ...
      it('should support single `expand`', () => {
        expect(service.parse({ expand: ['bar'] }).build())
          .toBe('/my-project1/foo?expand=bar')
      })
      it('should support mutliple `expand`s', () => {
        expect(service.parse({ expand: ['bar', 'baz'] }).build())
          .toBe('/my-project1/foo?expand=bar&expand=baz')
      })

      // query-id
      it('should support `id`', () => {
        expect(service.parse({ id: 'bar' }).build())
          .toBe('/my-project1/foo/bar')
      })
      it('should support `key`', () => {
        expect(service.parse({ key: 'bar' }).build())
          .toBe('/my-project1/foo/key=bar')
      })
      it('should support `customerId`', () => {
        expect(service.parse({ customerId: 'bar' }).build())
          .toBe('/my-project1/foo/?customerId=bar')
      })
    ...
    it('should support `page`', () => {
      expect(service.parse({ page: 1 }).build())
      .toBe('/my-project1/foo?offset=0')
    })
    it('should support `perPage`', () => {
      expect(service.parse({ perPage: 10 }).build())
      .toBe('/my-project1/foo?limit=10')
    })

    // query-projection
    it('should support `staged` being  `true`', () => {
      expect(service.parse({ staged: true }).build())
      .toBe('/my-project1/foo?staged=true')
    })
    it('should support `staged` being `false`', () => {
      expect(service.parse({ staged: false }).build())
      .toBe('/my-project1/foo?staged=false')
    })
    it('should support `priceCurrency`', () => {
      expect(service.parse({ priceCurrency: 'en' }).build())
      .toBe('/my-project1/foo?priceCurrency=en')
    })
    it('should support `priceCountry`', () => {
      expect(service.parse({ priceCountry: 'foo' }).build())
      .toBe('/my-project1/foo?priceCountry=foo')
    })
    it('should support `priceCustomerGroup`', () => {
      expect(service.parse({ priceCustomerGroup: 'foo' }).build())
      .toBe('/my-project1/foo?priceCustomerGroup=foo')
    })
    it('should support `priceChannel`', () => {
      expect(service.parse({ priceChannel: 'foo' }).build())
      .toBe('/my-project1/foo?priceChannel=foo')
    })
    ...
    it('should support a single `where` predicate', () => {
      expect(service.parse({ where: ['foo'] }).build())
        .toBe('/my-project1/foo?where=foo')
    })
    it('should support multiple `where` predicates', () => {
      expect(service.parse({ where: ['foo', 'bar'] }).build())
        .toBe('/my-project1/foo?where=foo%20and%20bar')
    })

    it('should support `version`', () => {
      expect(service.parse({ version: 2 }).build())
  ...
  describe('build', () => {
    ...
    it('endpoint with customer id', () => {
      expect(service.byCustomerId('cust123').build())
    ...
    it('endpoint with query params', () => {
      expect(service.expand('channel').build())
    ...
    it('full endpoint', () => {
      expect(service.byId('123').expand('channel').build())
