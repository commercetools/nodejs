...
describe('discountCodeGenerator', () => {
  ...
  test('should throw if no options or options is not valid', () => {
    expect(() => discountCodeGenerator()).toThrow(
      /The generator requires valid parameters/
    )
    expect(() => discountCodeGenerator('foo')).toThrow(
      /The generator requires valid parameters/
    )
  })

  test('should throw if no data object is passed', () => {
    expect(() => discountCodeGenerator(options)).toThrow(
      /The generator requires discount data/
    )
  })

  test('should throw if quantity is missing', () => {
    expect(() => discountCodeGenerator({ length: 23 }, data)).toThrow(
      /The generator requires valid parameters/
    )
  })
